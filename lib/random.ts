...
const stripCharsFromString = function (string: string, chars: string | any[]) {
  return string.split('').filter((char: any) => chars.indexOf(char) === -1)
}

export const int = randomInt

export function greyColor(min: number = 1, max: number = 9) {
  const int = randomInt(min, max).toString(16)

  return `#${int}${int}${int}`
}

export function captchaText(options?: any) {
...
const mathExprPlus = function (
  leftNumber: string | number,
 ...
 */
export function mathExpr(min: number = 1, max: number = 9, operator: string = "+") {
...
export function color(bgColor?: any) {
  // Random 24 colors
  // or based on step
